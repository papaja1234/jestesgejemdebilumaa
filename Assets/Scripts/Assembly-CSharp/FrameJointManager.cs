...
public class FrameJointManager : PartManager
{
	...
	private void AddFrameJoints(List<BasePart> parts)
	{
		...
		List<(BasePart, byte)> list3 = new List<(BasePart, byte)>();
		foreach (BasePart part2 in parts)
		{
			byte b = 0;
			BasePart enclosedPart = part2.m_enclosedPart;
			bool flag = enclosedPart != null && enclosedPart.m_partType == BasePart.PartType.SpringBoxingGlove && enclosedPart.customPartIndex == 5;
			if (part2.m_partType == BasePart.PartType.MetalFrame && flag)
			{
				b = (byte)(b | 1u);
			}
		...
		for (int l = 0; l < list3.Count; l++)
		{
			...
			{
				(BasePart, byte) tuple = list3[l];
				(BasePart, byte) tuple2 = list3[m];
				BasePart item = tuple.Item1;
				BasePart item2 = tuple2.Item1;
				byte num3 = (byte)(tuple.Item2 & tuple2.Item2);
				byte b2 = (byte)(num3 & 1u);
