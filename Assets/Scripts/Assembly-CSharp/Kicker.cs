...
public class Kicker : BasePart
{
	...
	public override bool IsTriggerable()
	{
		...
		if (INSettings.GetBool(INFeature.SeparatorConnection) && m_partTier != 0)
		{
			...
			for (int k = 0; k < m_connectedParts.Count; k++)
			{
				...
				if (basePart != null && m_isOverDistance[k])
				{
					...
					if (num6 > 1E-05f)
					{
						...
						float num7 = mass2 / (mass + mass2);
						float num8 = mass / (mass + mass2);
						float num9 = ((num6 > float2) ? ((float3 - num6) / (float3 - float2)) : 1f);
						float num10 = Vector.Dot(num4, num5, rightX, rightY) / num6;
						num10 *= num9;
						num10 = ((num10 > 20f) ? 20f : ((num10 < -20f) ? (-20f) : num10)) / num6;
						velocity -= new Vector3(num7 * num10 * num4, num7 * num10 * num5);
						basePart.rigidbody.velocity += new Vector3(num8 * num10 * num4, num8 * num10 * num5);
						float num11 = num6 - 1f;
						num11 *= num9;
						num11 = ((num11 > 0.4f) ? 0.4f : ((num11 < -0.4f) ? (-0.4f) : num11)) / num6;
						velocity -= new Vector3(num7 * num11 * num4 / fixedDeltaTime, num7 * num11 * num5 / fixedDeltaTime);
						basePart.rigidbody.velocity += new Vector3(num8 * num11 * num4 / fixedDeltaTime, num8 * num11 * num5 / fixedDeltaTime);
					}
				}
			}
		...
		else
		{
			...
			for (int l = 0; l < m_connectedParts.Count; l++)
			{
				...
				if (basePart2 != null && m_isOverDistance[l])
				{
					...
					float num12 = Vector.Distance(position5, position6);
					float num13 = ((num12 > float2) ? ((float3 - num12) / (float3 - float2)) : 1f);
					float num14 = 32f * (velocity3.x - velocity4.x) + 128f * (position5.x - position6.x);
					float num15 = 32f * (velocity3.y - velocity4.y) + 128f * (position5.y - position6.y);
					num14 *= num13;
					num15 *= num13;
					float num16 = Mathf.Sqrt(num14 * num14 + num15 * num15);
					float mass4 = basePart2.rigidbody.mass;
					float num17 = ((num16 * mass4 > 250f) ? (deltaTime * 250f / (num16 * mass4)) : deltaTime);
					float num18 = num17 * mass4 / mass3;
					velocity3 -= new Vector3(num18 * num14, num18 * num15);
					basePart2.rigidbody.velocity += new Vector3(num17 * num14, num17 * num15);
				}
			}
