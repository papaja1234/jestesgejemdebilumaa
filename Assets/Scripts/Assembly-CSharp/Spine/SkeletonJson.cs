	...
	public class SkeletonJson
	{
		...
		private void ReadAnimation(Dictionary<string, object> map, string name, SkeletonData skeletonData)
		{
				...
				foreach (KeyValuePair<string, object> item7 in (Dictionary<string, object>)map["bones"])
				{
					...
					foreach (KeyValuePair<string, object> item8 in (Dictionary<string, object>)item7.Value)
					{
						...
						case "rotate":
						{
							...
							{
								rotateTimeline.SetFrame(num7, (float)item9["time"], (float)item9["angle"]);
								ReadCurve(item9, rotateTimeline, num7);
								num7++;
							}
						...
						case "shear":
						{
							...
							foreach (Dictionary<string, object> item10 in list2)
							{
								...
								translateTimeline.SetFrame(num6, time3, @float * num5, float2 * num5);
			...
			if (map.ContainsKey("ik"))
			{
				foreach (KeyValuePair<string, object> item11 in (Dictionary<string, object>)map["ik"])
				{
					...
					foreach (Dictionary<string, object> item12 in obj3)
					{
						...
						ReadCurve(item12, ikConstraintTimeline, num8);
						num8++;
					}
			...
			if (map.ContainsKey("transform"))
			{
				foreach (KeyValuePair<string, object> item13 in (Dictionary<string, object>)map["transform"])
				{
					...
					foreach (Dictionary<string, object> item14 in obj5)
					{
						...
						ReadCurve(item14, transformConstraintTimeline, num9);
						num9++;
					}
			...
			if (map.ContainsKey("paths"))
			{
				...
				{
					int num10 = skeletonData.FindPathConstraintIndex(item15.Key);
					if (num10 == -1)
					{
					...
					foreach (KeyValuePair<string, object> item16 in (Dictionary<string, object>)item15.Value)
					{
						...
						case "spacing":
						{
							...
							{
								pathConstraintPositionTimeline.SetFrame(num13, (float)item17["time"], GetFloat(item17, key5, 0f) * num12);
								ReadCurve(item17, pathConstraintPositionTimeline, num13);
								num13++;
							}
						...
						case "mix":
						{
							...
							{
								pathConstraintMixTimeline.SetFrame(num11, (float)item18["time"], GetFloat(item18, "rotateMix", 1f), GetFloat(item18, "translateMix", 1f));
								ReadCurve(item18, pathConstraintMixTimeline, num11);
								num11++;
							}
			...
			if (map.ContainsKey("deform"))
			{
				foreach (KeyValuePair<string, object> item19 in (Dictionary<string, object>)map["deform"])
				{
					...
					{
						int num14 = skeletonData.FindSlotIndex(item20.Key);
						if (num14 == -1)
						{
						...
						foreach (KeyValuePair<string, object> item21 in (Dictionary<string, object>)item20.Value)
						{
							...
							VertexAttachment vertexAttachment = (VertexAttachment)skin.GetAttachment(num14, item21.Key);
							...
							foreach (Dictionary<string, object> item22 in obj7)
							{
								...
								else
								{
									...
									{
										int i = @int;
										for (int num17 = i + floatArray.Length; i < num17; i++)
										{
											array[i] *= scale;
										}
									}
								...
								deformTimeline.SetFrame(num16, (float)item22["time"], array);
								ReadCurve(item22, deformTimeline, num16);
								num16++;
							}
			...
			if (map.ContainsKey("drawOrder") || map.ContainsKey("draworder"))
			{
				...
				foreach (Dictionary<string, object> item23 in obj8)
				{
					...
					if (item23.ContainsKey("offsets"))
					{
						...
						{
							int num22 = skeletonData.FindSlotIndex((string)item24["slot"]);
							if (num22 == -1)
							{
							...
							int num23 = num20 + (int)(float)item24["offset"];
							array2[num23] = num20++;
						}
						while (num20 < count)
						{
						...
						for (int num24 = count - 1; num24 >= 0; num24--)
						{
							if (array2[num24] == -1)
							{
								array2[num24] = array3[--num21];
							}
						}
					}
					drawOrderTimeline.SetFrame(num18++, (float)item23["time"], array2);
				}
			...
			if (map.ContainsKey("events"))
			{
				...
				foreach (Dictionary<string, object> item25 in obj9)
				{
					...
					eventTimeline.SetFrame(num25++, @event);
				}
