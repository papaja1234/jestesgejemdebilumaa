...
public class LevelComplete : WPFMonoBehaviour
{
	...
	{
		for (int k = 0; k < Singleton<GameManager>.Instance.gameData.m_episodeLevels.Count; k++)
		{
			if (Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].Name == Singleton<GameManager>.Instance.CurrentEpisode)
			{
				if (Singleton<GameManager>.Instance.IsLastLevelInEpisode() && Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].ClearAchievement != null)
				{
					Singleton<SocialGameManager>.Instance.ReportAchievementProgress(Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].ClearAchievement, 100.0);
				}
				if (Singleton<GameManager>.Instance.CurrentEpisodeThreeStarredNormalLevels() && Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].ThreeStarAchievement != null)
				{
					Singleton<SocialGameManager>.Instance.ReportAchievementProgress(Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].ThreeStarAchievement, 100.0);
				}
				if (Singleton<GameManager>.Instance.CurrentEpisodeThreeStarredSpecialLevels() && Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].SpecialThreeStarAchievement != null)
				{
					Singleton<SocialGameManager>.Instance.ReportAchievementProgress(Singleton<GameManager>.Instance.gameData.m_episodeLevels[k].SpecialThreeStarAchievement, 100.0);
				}
			}
		}
		if (WPFMonoBehaviour.levelManager.IsPartTransported(BasePart.PartType.KingPig))
		{
			int j = GameProgress.GetInt("Transported_Kings");
			j++;
			GameProgress.SetInt("Transported_Kings", j);
			((Action<List<string>>)delegate(List<string> achievements)
			{
				foreach (string achievement in achievements)
				{
					if (Singleton<SocialGameManager>.Instance.TryReportAchievementProgress(achievement, 100.0, (int limit) => j >= limit))
					{
						break;
					}
				}
		...
		{
			int i = GameProgress.GetInt("Perfect_Completions");
			i++;
			GameProgress.SetInt("Perfect_Completions", i);
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.SKILLED_PILOT", 100.0, (int limit) => i >= limit);
		}
