...
public class SecretPlace : OneTimeCollectable
{
	...
	public override void OnCollected()
	{
		...
		int i = GameProgress.SecretSkullCount();
		if (Singleton<SocialGameManager>.IsInstantiated())
		{
			...
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.IS_IT_SECRET", 100.0, (int limit) => i >= limit);
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.SECRET_ADMIRER", 100.0, (int limit) => i >= limit);
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.MASTER_EXPLORER", 100.0, (int limit) => i >= limit);
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.MEGA_MASTER_EXPLORER", 100.0, (int limit) => i >= limit);
		}
	}
