...
namespace PlayFab.Internal
{
	public class PlayFabUnityHttp : IPlayFabHttp
	{
		...
		private static IEnumerator SimpleCallCoroutine(string fullUrl, byte[] payload, Action<byte[]> successCallback, Action<string> errorCallback)
		{
			if (payload == null)
			{
				UnityWebRequest www2 = new UnityWebRequest(fullUrl)
				{
			...
			{
				UnityWebRequest www2 = UnityWebRequest.Put(fullUrl, payload);
				www2.SendWebRequest();
				while (www2.uploadProgress < 1f && www2.downloadProgress < 1f)
				{
					yield return 1;
				}
				if (!string.IsNullOrEmpty(www2.error))
