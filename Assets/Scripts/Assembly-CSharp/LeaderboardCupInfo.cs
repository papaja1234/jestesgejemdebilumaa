...
public class LeaderboardCupInfo : MonoBehaviour
{
	...
	private IEnumerator AnimEnableCup(PlayFabLeaderboard.Leaderboard cup, bool enable)
	{
		...
		int cupIndex2 = (int)cup;
		cupIndex2--;
		if (cupIndex2 < 0 || cupIndex2 >= cups.Length || !(cups[cupIndex2] != null))
		{
			yield break;
		}
		Transform currentCupGraphics = cups[cupIndex2].Find("BarYou");
		Transform shine = cups[cupIndex2].Find("Shine");
		...
		else if (enable && (bool)shine)
		{
			...
			GameObject gameObject = cups[cupIndex2].Find("CupIcon").gameObject;
