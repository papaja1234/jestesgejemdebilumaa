	...
	private void FixedUpdate()
	{
		...
		if (m_open)
		{
			...
			else if (flag && m_partTier == PartTier.Epic)
			{
				...
				float num10 = m_angle + m_deltaAngle;
				float num11 = num10 - Mathf.Atan2(up2.y, up2.x);
				num11 = ((num11 < 0f - num7) ? (num11 + 2f * num7) : ((num11 > num7) ? (num11 - 2f * num7) : num11));
				float num12 = -32f * z2 + 400f * num11;
				num12 *= ((customPartIndex == 3) ? 1f : 1.5f);
				Vector3 vector = ((customPartIndex == 3) ? up2 : ((Vector3)new Vector2(Mathf.Cos(num10), Mathf.Sin(num10))));
				vector = new Vector3(0f - vector.y, vector.x);
				float num13 = (vector.x * velocity2.x + vector.y * velocity2.y) * num12;
				num12 *= ((num13 > num9) ? (num9 / num13) : 1f);
				num12 = Math.Clamp(num12, 0f - num8, num8);
				base.rigidbody.AddForce(new Vector3(vector.x * num12, vector.y * num12), ForceMode.Force);
				base.rigidbody.AddTorque(new Vector3(0f, 0f, -64f * z2 + 1024f * num11), ForceMode.Acceleration);
			}
			else
			{
				...
				float num14 = Vector3.Dot(base.transform.up, vector2);
				float num15 = Mathf.Sign(Vector3.Cross(vector2, base.transform.right).z);
				float num16 = num15 * num14 * num14;
				if (Mathf.Abs(num16) > 60f)
				{
					num16 = 60f * Mathf.Sign(num16);
				}
				Vector3 up3 = base.transform.up;
				base.rigidbody.AddForceAtPosition(num16 * up3, position, ForceMode.Force);
				num14 = Vector3.Dot(base.transform.right, vector2);
				num15 = Mathf.Sign(Vector3.Cross(base.transform.up, vector2).z);
				num16 = 0.5f * num15 * num14 * num14;
				if (Mathf.Abs(num16) > 60f)
				{
					num16 = 60f * Mathf.Sign(num16);
				}
		...
		else
		{
			...
			float num17 = -0.2f * m_maximumForce;
			base.rigidbody.AddForce(num17 * base.transform.up, ForceMode.Force);
		}
