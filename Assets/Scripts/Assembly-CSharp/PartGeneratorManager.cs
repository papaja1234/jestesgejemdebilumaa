			...
			for (int i = start; i <= end; i++)
			{
				...
				if (part is ElectricalPart electricalPart)
				{
					...
					for (int l = 0; l < 4; l++)
					{
						...
						if (num7 != -1)
						{
							BasePart basePart4 = runtimeContraption.Parts[num7].Part;
				...
				for (int m = 0; m < 4; m++)
				{
					int num11 = runtimeContraption.FindPartIndexAt(coordX + num9, coordY + num10);
					if (num11 != -1)
					{
						BasePart part3 = runtimeContraption.Parts[num11].Part;
						if (instance.CanConnectTo(part, part3, (BasePart.Direction)m))
						{
							m_disjointSet.Union(i, num11);
						}
					}
					int num12 = num9;
					num9 = -num10;
